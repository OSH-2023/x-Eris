...
##### FreeRTOS移植
移植分为两步，首先需配置`FreeRTOSConfig.h`文件，以实现对FreeRTOS系统的裁剪并实现对该开发板的支持。
随后修改开发板官方提供的`SYSTEM`文件，将对操作系统的选用换为FreeRTOS，并更改系统时钟，中断等操作以使得FreeRTOS可以正常运行。

##### 串口通信
为烧录程序并得到运行结果，采用串口方式进行通信。
程序在编译成hex文件后借助FlyMcu烧入开发板。
对于硬件，加入`_sys_exit_(int x){x=x;}`限制其以避免进入半主机模式，并将`printf`输出到usart1串口。
接收数据则通过XCOM串口调试助手实现。
